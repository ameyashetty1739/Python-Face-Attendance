...
def output(filename, sheet,num, name, present):
    my_file = Path('C:\miniproj1/'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('C:\miniproj1/'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
    else:
        book = xlwt.Workbook()
    ...
    book.save('C:\miniproj1/'+fullname)
    return fullname;
